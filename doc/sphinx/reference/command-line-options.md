...
Colon separated list of additional environment variables add/modify/remove when launching X11 applications. Format is the same as as `--app-env`. Changes are applied after `--app-env` and `--app-env-amend`.
