...
2. Next, clone Wayland Explorer:
    ```sh
    git clone git@github.com:vially/wayland-explorer.git
    ```

2. Next, run `miral-app` with all available Wayland extensions enbled:
    ```sh
    WAYLAND_DISPLAY=wayland-98 miral-app --add-wayland-extension=all
    ```

3. Then, run `wlprobe` from the `wayland-explorer` directory:
    ```sh
    cd wayland-explorer
...
4. Finally, check the changes and open up a pull request
    ```sh
