...
template<>
struct std::formatter<MirTouchpadConfig> : std::formatter<std::string>
{
    auto format(MirTouchpadConfig const& conf, std::format_context& ctx) const -> std::format_context::iterator;
};
