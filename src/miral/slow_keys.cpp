...
miral::SlowKeys& miral::SlowKeys::on_enabled(std::function<void()>&& on_enabled)
{
    auto const state = self->state.lock();
    state->on_enabled = std::move(on_enabled);
    if (auto const accessibility_manager = self->accessibility_manager.lock())
        accessibility_manager->slow_keys().on_enabled(std::move(state->on_enabled));

    return *this;
}

miral::SlowKeys& miral::SlowKeys::on_disabled(std::function<void()>&& on_disabled)
{
    auto const state = self->state.lock();
    state->on_disabled = std::move(on_disabled);
    if (auto const accessibility_manager = self->accessibility_manager.lock())
        accessibility_manager->slow_keys().on_disabled(std::move(state->on_disabled));
