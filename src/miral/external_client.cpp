...
void miral::ExternalClientLauncher::operator()(mir::Server& server)
{
    ...
    static auto const default_env =
        "GDK_BACKEND=wayland,x11:QT_QPA_PLATFORM=wayland:SDL_VIDEODRIVER=wayland:"
    ...
    server.add_configuration_option(app_env, "Environment for launched apps", default_env);

    server.add_configuration_option(app_x11_env, "X11 changes to --app-env for launched apps", default_x11_env);
