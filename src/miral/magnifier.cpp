...
struct State
{
    ...
    void apply_geometry(mml::FreePlacement const& new_placement)
    {
        ...
        freely_positioned_center = geom::PointD{
            new_placement.surface_top_left + geom::generic::as_displacement(new_placement.capture_area.size / 2.0)};
    ...
    std::optional<miral::Magnifier::Behavior> config_behavior;
...
private:
    ...
    private:
        ...
        { self->set_magnification(s, std::clamp(s.magnification + delta, min_magnification, max_magnification)); }
    ...
    : Magnifier()
{
    ...
    config_store.add_string_attribute(
        ...
        [this](live_config::Key const&, std::optional<std::string_view> val)
        {
            ...
            if (!self->config_changed(&State::config_behavior, *behavior))
                return;
