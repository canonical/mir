...
PixelFormatSelector::PixelFormatSelector(std::shared_ptr<mg::DisplayConfigurationPolicy> const& base_policy,
                                         bool with_alpha) :
...
void PixelFormatSelector::apply_to(mg::DisplayConfiguration& conf)
{
    ...
    conf.for_each_output([&](mg::UserDisplayConfigurationOutput& conf_output) {
        ...
        auto const& pos = find_if(conf_output.pixel_formats.begin(),
                                  ...
                                  [&](MirPixelFormat format) -> bool { return contains_alpha(format) == with_alpha; });
