...
struct miral::tk::WaylandShmPool
{
public:
...
miral::tk::WaylandShm::WaylandShm(wl_shm* shm)
    : shm{shm}
{
}

auto miral::tk::WaylandShm::get_buffer(geom::Size const& size, geom::Stride const& stride) -> std::shared_ptr<WaylandShmBuffer>
{
    ...
    if (!free_buffer)
    {
        ...
        free_buffer = std::make_shared<WaylandShmBuffer>(pool, data, size, stride, buffer);
