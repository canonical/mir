...
miral::ConfigFile::Self::Self(MirRunner& runner, path file, Mode mode, OverrideLoader load_multi_configs)
{
    if(auto const config_file = find_config_file(file))
    {
        ...
        std::move(load_multi_configs)(config_streams);
    ...
    case Mode::reload_on_change:
        override_watcher = std::make_shared<OverrideWatcher>(std::move(file), std::move(load_multi_configs));
...
miral::ConfigFile::ConfigFile(MirRunner& runner, path file, Mode mode, OverrideLoader load_config) :
    self{std::make_shared<Self>(runner, std::move(file), mode, std::move(load_config))}
{
}
...
void OverrideWatcher::handler(int)
{
    ...
    while (raw_buffer != buffer + readsize)
    {
        ...
        auto const override_directory_created =
            create && is_base_config_directory_event && event.name == override_directory;
        auto const override_directory_removed =
            remove && is_base_config_directory_event && event.name == override_directory;

        auto const is_conf_file = std::string_view{event.name}.ends_with(".conf");
        auto const override_file_changed = (write_or_move || remove) && is_override_directory_event && is_conf_file;

        if (base_config_changed || override_file_changed || override_directory_created || override_directory_removed)
            try
            {
                ...
                if (base_config_exists)
                {
                    ...
                    if (override_directory_created)
                    {
                        if (override_directory_watch_descriptor
                            && inotify_rm_watch(inotify_fd, *override_directory_watch_descriptor) != 0)
                            mir::log_warning(
                        ...
                        // Check if the override directory has any files, if not, return and  don't reload
                        if (is_empty(base_config_directory.value() / override_directory))
                        {
                            mir::log_debug(
                                "Override directory '%s' created but contains no .conf files, skipping reload",
                                override_directory.c_str());
                            return;
                        }
                    }

                    if(override_directory_removed)
                    {
                        if (override_directory_watch_descriptor
                            && inotify_rm_watch(inotify_fd, *override_directory_watch_descriptor) != 0)
                            mir::log_warning(
                        ...
                        override_directory_watch_descriptor.reset();
                    }
