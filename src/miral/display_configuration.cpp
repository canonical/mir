...
auto miral::DisplayConfiguration::Node::has(std::string_view key) const -> bool
{
    if (type() != Type::map)
...
auto miral::DisplayConfiguration::Node::at(std::string_view key) const -> std::optional<Node>
{
    if (!has(key))
        return std::nullopt;
