...
#define APPEND(field) bout.append(#field, info.field());
        ...
        if (info.min_width() != miral::default_min_width)
            APPEND(min_width);
        if (info.min_height() != miral::default_min_height)
            APPEND(min_height);
        if (info.max_width() != miral::default_max_width)
            APPEND(max_width);
        if (info.max_height() != miral::default_max_height)
            APPEND(max_height);
        if (info.width_inc() != miral::default_width_inc)
            APPEND(width_inc);
        if (info.height_inc() != miral::default_height_inc)
            APPEND(height_inc);
        if (info.min_aspect() != miral::default_min_aspect_ratio)
            APPEND(min_aspect);
        if (info.max_aspect() != miral::default_max_aspect_ratio)
            APPEND(max_aspect);
        if (info.depth_layer() != mir_depth_layer_application)
            APPEND(depth_layer);
        if (info.attached_edges() != 0)
            APPEND(attached_edges);
        if (info.exclusive_rect() != mir::geometry::Rectangle{{}, {}})
            APPEND(exclusive_rect);
        APPEND(preferred_orientation);
        APPEND(confine_pointer);

#define APPEND_IF_SET(field) \
    if (info.has_##field()) \
        bout.append(#field, info.field());
...
#define APPEND_IF_SET(field) \
    if (specification.field().is_set()) \
        bout.append(#field, specification.field().value());
        APPEND_IF_SET(name);
        APPEND_IF_SET(type);
...
auto dump_of(MirPointerEvent const* event) -> std::string
{
    ...
    for (auto const a : {mir_pointer_button_primary,
...
miral::WindowManagementTrace::WindowManagementTrace(WindowManagerTools const& wrapped,
...
void miral::WindowManagementTrace::place_and_size_for_state(WindowSpecification& modifications,
                                                            WindowInfo const& window_info) const
try
{
    log_input();
    mir::log_info(
        "%s modifications=%s window_info=%s", __func__, dump_of(modifications).c_str(), dump_of(window_info).c_str());
...
void miral::WindowManagementTrace::start_drag_and_drop(miral::WindowInfo& window_info,
...
void miral::WindowManagementTrace::modify_window(miral::WindowInfo& window_info,
                                                 miral::WindowSpecification const& modifications)
try
{
    log_input();
    mir::log_info(
        "%s window_info=%s, modifications=%s", __func__, dump_of(window_info).c_str(), dump_of(modifications).c_str());
    trace_count++;
...
void miral::WindowManagementTrace::add_tree_to_workspace(miral::Window const& window,
                                                         std::shared_ptr<miral::Workspace> const& workspace)
try
{
    mir::log_info(
        "%s window=%s, workspace =%p", __func__, dump_of(window).c_str(), static_cast<void*>(workspace.get()));
    wrapped.add_tree_to_workspace(window, workspace);
}
MIRAL_TRACE_EXCEPTION

void miral::WindowManagementTrace::remove_tree_from_workspace(miral::Window const& window,
                                                              std::shared_ptr<miral::Workspace> const& workspace)
try
{
    mir::log_info(
...
void miral::WindowManagementTrace::move_workspace_content_to_workspace(std::shared_ptr<Workspace> const& to_workspace,
                                                                       std::shared_ptr<Workspace> const& from_workspace)
try
{
    mir::log_info("%s to_workspace=%p, from_workspace=%p",
                  __func__,
...
auto miral::WindowManagementTrace::place_new_window(ApplicationInfo const& app_info,
...
try
{
    ...
    mir::log_info("%s app_info=%s, requested_specification=%s -> %s",
                  __func__,
                  dump_of(app_info).c_str(),
                  dump_of(requested_specification).c_str(),
                  dump_of(result).c_str());
    return result;
}
MIRAL_TRACE_EXCEPTION
...
void miral::WindowManagementTrace::handle_modify_window(miral::WindowInfo& window_info,
                                                        miral::WindowSpecification const& modifications)
try
{
    mir::log_info(
...
void miral::WindowManagementTrace::advise_resize(miral::WindowInfo const& window_info,
                                                 mir::geometry::Size const& new_size)
try
{
...
void miral::WindowManagementTrace::handle_request_resize(miral::WindowInfo& window_info,
...
void miral::WindowManagementTrace::advise_adding_to_workspace(std::shared_ptr<miral::Workspace> const& workspace,
                                                              std::vector<miral::Window> const& windows)
try
{
    mir::log_info(
        "%s workspace=%p, windows=%s", __func__, static_cast<void*>(workspace.get()), dump_of(windows).c_str());
    policy->advise_adding_to_workspace(workspace, windows);
}
MIRAL_TRACE_EXCEPTION

void miral::WindowManagementTrace::advise_removing_from_workspace(std::shared_ptr<miral::Workspace> const& workspace,
                                                                  std::vector<miral::Window> const& windows)
try
{
    mir::log_info(
...
auto miral::WindowManagementTrace::confirm_placement_on_display(WindowInfo const& window_info,
...
try
{
    ...
    mir::log_info("%s window_info=%s, new_state= %s, new_placement= %s -> %s",
                  __func__,
                  dump_of(window_info).c_str(),
                  dump_of(new_state).c_str(),
                  dump_of(new_placement).c_str(),
                  dump_of(result).c_str());
    return result;
}
MIRAL_TRACE_EXCEPTION
