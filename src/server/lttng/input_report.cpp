...
void mir::lttng::InputReport::published_key_event(int dest_fd, uint32_t seq_id, int64_t event_time)
{
    mir_tracepoint(mir_server_input, published_key_event, dest_fd, seq_id, event_time);
}

void mir::lttng::InputReport::published_motion_event(int dest_fd, uint32_t seq_id, int64_t event_time)
{
    mir_tracepoint(mir_server_input, published_motion_event, dest_fd, seq_id, event_time);
}

void mir::lttng::InputReport::received_event_finished_signal(int src_fd, uint32_t seq_id)
{
