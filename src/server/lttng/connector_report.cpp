...
void mir::lttng::ConnectorReport::starting_threads(int count)
{
    mir_tracepoint(mir_server_connector, starting_threads, count);
}

void mir::lttng::ConnectorReport::stopping_threads(int count)
{
    mir_tracepoint(mir_server_connector, stopping_threads, count);
}

void mir::lttng::ConnectorReport::creating_session_for(int socket_handle)
{
    mir_tracepoint(mir_server_connector, creating_session_for, socket_handle);
}

void mir::lttng::ConnectorReport::creating_socket_pair(int server_handle, int client_handle)
{
    mir_tracepoint(mir_server_connector, creating_socket_pair, server_handle, client_handle);
}

void mir::lttng::ConnectorReport::listening_on(std::string const& endpoint)
{
    mir_tracepoint(mir_server_connector, listening_on, endpoint.c_str());
}

void mir::lttng::ConnectorReport::error(std::exception const& error)
{
