      ...
      scale{scale}
{
    ...
    {
        log_debug("Created XWaylandSurfaceRole for wl_surface@{}", wl_resource_get_id(wl_surface->resource));
    }
}
...
mf::XWaylandSurfaceRole::~XWaylandSurfaceRole()
{
    ...
    if (verbose_xwayland_logging_enabled())
    {
        log_debug("Destroyed XWaylandSurfaceRole for wl_surface@{}", wl_resource_get_id(wl_surface->resource));
    }
}
