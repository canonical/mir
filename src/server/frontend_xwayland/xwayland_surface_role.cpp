      ...
      scale{scale}
{
    ...
    {
        log_debug("Created XWaylandSurfaceRole for wl_surface@%u", wl_surface->object_id());
    }
}
...
mf::XWaylandSurfaceRole::~XWaylandSurfaceRole()
{
    ...
    if (verbose_xwayland_logging_enabled())
    {
        log_debug("Destroyed XWaylandSurfaceRole for wl_surface@%u", wl_surface->object_id());
    }
}
