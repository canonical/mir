      ...
      output_global{mw::make_weak(&output_global)}
{
    ...
    if (wl_resource_get_version(resource) >= 3)
    {
        ...
        {
            log_warning("xdg_output_v1 is v{} (meaning Mir must send wl_output.done), "
                "but wl_output is only v{} (meaning it can't)", wl_resource_get_version(resource), wl_resource_get_version(wl_output_resource));
        }
    }
}
