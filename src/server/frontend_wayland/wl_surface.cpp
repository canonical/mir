    ...
    if (role)
    {
        if (auto const scene_surface = role->scene_surface())
        {
            if (pending.orientation)
                scene_surface.value()->set_orientation(pending.orientation.value());
            if (pending.mirror_mode)
                scene_surface.value()->set_mirror_mode(pending.mirror_mode.value());
        }
     }
...
{
    std::optional<std::tuple<MirOrientation, MirMirrorMode>> const result = OutputManager::from_output_transform(transform);
    if (!result)
    {
        throw wayland::ProtocolError{
            resource,
    ...
    pending.orientation = std::get<0>(result.value());
    pending.mirror_mode = std::get<1>(result.value());
}
