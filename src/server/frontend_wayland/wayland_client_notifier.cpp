...
auto mf::WaylandClientNotifier::client_added(rust::Box<mwrs::WaylandClient> wayland_client) -> void
{
    try
    {
        ...
        if (!session_authorizer->connection_is_allowed({pid, uid, gid}))
        {
            ...
            // wl_display error code for an authorizer rejection, so we use the
            // `implementation` code, mirroring how compositor-side failures are
            // reported elsewhere in the frontend.
            uint32_t constexpr wl_display_object_id = 1;
            uint32_t constexpr wl_display_error_implementation = 3;
            wayland_client->kill(
                wl_display_object_id, wl_display_error_implementation, "client rejected by session authorizer");
            return;
        }
