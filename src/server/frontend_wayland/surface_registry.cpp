...
void mf::SurfaceRegistry::add_surface(
    std::shared_ptr<scene::Surface const> const& surf, mw::Weak<mf::WlSurface> const& wl_surf)
{
    scene_surface_to_wayland_surface.insert({surf, wl_surf});
}

void mf::SurfaceRegistry::remove_surface(std::shared_ptr<scene::Surface const> const& surf)
{
    scene_surface_to_wayland_surface.erase(surf);
}

auto mf::SurfaceRegistry::lookup_wayland_surface(std::shared_ptr<scene::Surface const> const& surf)
