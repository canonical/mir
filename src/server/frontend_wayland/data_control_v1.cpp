        ...
        std::make_shared<ClipboardObserver>([this](auto source) { on_clipboard_set(source, true); })}
{
    ...
    if (s->current_source)
    {
        auto offer = new mf::DataControlOfferV1(wayland::Weak{this}, s->current_source.value().mime_types(), false);
    ...
    {
        auto offer =
            new mf::DataControlOfferV1(wayland::Weak{this}, s->current_primary_source.value().mime_types(), true);
