...
bool mf::SessionLockManagerV1::try_lock(SessionLockV1* lock)
{
    ...
    {
        mir::log_info("SessionLockManagerV1 has successfully processed a lock");
        active_lock = lock;
        session_lock->lock();
        return true;
    }

    mir::log_warning("SessionLockManagerV1 has failed to process a lock");
    return false;
}
...
bool mf::SessionLockManagerV1::try_unlock(SessionLockV1* lock)
{
    ...
    {
        mir::log_info("SessionLockManagerV1 has successfully processed an unlock");
        session_lock->unlock();
        return true;
    }

    mir::log_warning("SessionLockManagerV1 has failed to process an unlock");
    return false;
}
      ...
      observer{std::make_shared<SessionLockV1Observer>(*this)}
{
    ...
    {
        mir::log_info("SessionLockV1 successfully acquired the lock");
        send_locked_event();
        manager.session_lock->register_interest(observer, manager.wayland_executor);
    }
    else
    {
        mir::log_info("SessionLockV1 failed to acquire the lock");
        send_finished_event();
    }
}
...
{
    mir::log_info("SessionLockV1 is dying");
...
void mf::SessionLockV1::destroy()
{
    ...
    {
        manager.try_relinquish_locking_privilege(this);
        mir::log_info("SessionLockV1 successfully destroyed itself");
    }
    else
    {
        mir::log_info("SessionLockV1 attempted to destroy itself when it was not the lock");
...
void mf::SessionLockV1::unlock_and_destroy()
{
    ...
    {
        mir::log_info("SessionLockV1 attempted to unlock_and_destroy but it is not allowed to");
        BOOST_THROW_EXCEPTION(mw::ProtocolError(
            resource,
    ...
    {
        manager.try_relinquish_locking_privilege(this);
        mir::log_info("SessionLockV1 successfully unlocked_and_destroyed itself");
    }
}
