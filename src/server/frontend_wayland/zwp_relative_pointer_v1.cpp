...
namespace frontend
{
class RelativePointerManagerV1 : public wayland_rs::RelativePointerManagerV1
{
public:
...
private:
    ...
    using wayland_rs::RelativePointerManagerV1::get_relative_pointer;
    auto get_relative_pointer(
        wayland_rs::Weak<wayland_rs::Pointer> const& pointer,
        rust::Box<wayland_rs::RelativePointerV1Middleware> child_instance,
        uint32_t child_object_id) -> std::shared_ptr<wayland_rs::RelativePointerV1> override;
};

class RelativePointerV1 : public wayland_rs::RelativePointerV1
{
public:
    RelativePointerV1(
        std::shared_ptr<wayland_rs::Client> client,
        rust::Box<wayland_rs::RelativePointerV1Middleware> instance,
        uint32_t object_id,
        WlPointer* pointer);

private:
    wayland_rs::Weak<WlPointer> const pointer;
};
}
}
...
mir::frontend::RelativePointerManagerV1::RelativePointerManagerV1(
    ...
    uint32_t object_id,
    std::shared_ptr<shell::Shell> shell) :
    wayland_rs::RelativePointerManagerV1{std::move(client), std::move(instance), object_id},
...
auto mir::frontend::RelativePointerManagerV1::get_relative_pointer(
    wayland_rs::Weak<wayland_rs::Pointer> const& pointer,
    rust::Box<wayland_rs::RelativePointerV1Middleware> child_instance,
    uint32_t child_object_id) -> std::shared_ptr<wayland_rs::RelativePointerV1>
{
    return std::make_shared<RelativePointerV1>(
        client, std::move(child_instance), child_object_id,
        wayland_rs::Pointer::from<WlPointer>(pointer));
}
...
mir::frontend::RelativePointerV1::RelativePointerV1(
    ...
    uint32_t object_id,
    WlPointer* pointer) :
    wayland_rs::RelativePointerV1{std::move(client), std::move(instance), object_id},
