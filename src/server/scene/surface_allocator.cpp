...
{
    auto confine = params.confine_pointer.has_value() ? params.confine_pointer.value() : mir_pointer_unconfined;
    auto const name = params.name.has_value() ? params.name.value() : "";
    ...
    auto const parent = params.parent.has_value() ? params.parent.value() : std::weak_ptr<scene::Surface>{};
