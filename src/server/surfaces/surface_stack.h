...
public:
    ...
    virtual std::weak_ptr<shell::BasicSurface> create_surface(const shell::SurfaceCreationParameters& params);

    virtual void destroy_surface(std::weak_ptr<shell::BasicSurface> const& surface);
    
    virtual void raise(std::weak_ptr<shell::BasicSurface> const& surface);
...
private:
    ...
    typedef std::vector<std::shared_ptr<shell::BasicSurface>> Layer;
