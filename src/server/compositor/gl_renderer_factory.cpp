...
public:
    GLResources()
        : vertex_shader{0}, fragment_shader{0},
          program{0}, position_attr_loc{0},
          texcoord_attr_loc{0}, transform_uniform_loc{0},
          alpha_uniform_loc{0}, vertex_attribs_vbo{0}, texture{0}
    {
    }
    ...
    void setup(geom::Size const& display_size)
    {
        GLint param = 0;
...
class GLRenderer : public mc::Renderer
{
public:
    GLRenderer(geom::Size const& display_size)
    {
        resources.setup(display_size);
    }

    void render(std::function<void(std::shared_ptr<void> const&)> save_resource,
                                   mc::CompositingCriteria const& criteria,
                                   ms::BufferStream& stream)
    {
        ...
        glVertexAttribPointer(resources.position_attr_loc, 3, GL_FLOAT, GL_FALSE,
                              sizeof(VertexAttributes), 0);
        glVertexAttribPointer(resources.texcoord_attr_loc, 2, GL_FLOAT, GL_FALSE,
