...
public:
    CompositingFunctor(
        ...
        std::shared_ptr<mc::DebugDrawManager> const& debug_draw_manager) :
            ...
            while (running)
            {
                ...
                if (running)
                {
                    ...
                    for (auto& tuple : compositors)
                    {
                        ...
                        scene_elements.append_range(debug_draw_manager->process_commands());
...
void mc::MultiThreadedCompositor::create_compositing_threads()
{
    ...
    display->for_each_display_sync_group([this](mg::DisplaySyncGroup& group)
    {
        auto thread_functor = std::make_unique<mc::CompositingFunctor>(
            ...
            fixed_composite_delay, report, cursor, debug_draw_manager);
