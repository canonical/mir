...
struct msd::InputResolver::InputResolver::Observer : public mir::scene::NullSurfaceObserver
{
    ...
    Observer(InputResolver* parent);

    void input_consumed(mir::scene::Surface const*, std::shared_ptr<MirEvent const> const& event) override;
    InputResolver* parent;
};

msd::InputResolver::Observer::Observer(InputResolver* parent) :
    parent{parent}
{
}

void msd::InputResolver::Observer::input_consumed(
    mir::scene::Surface const*, std::shared_ptr<MirEvent const> const& event)
{
                ...
                case mir_touch_action_change:
                    {
                        ...
                        {
                            parent->touch_event(event, id, geometry::Point{position.value()});
                        }
                    }
                    break;
                case mir_touch_action_up:
                    {
                        parent->touch_up(event, id);
                        break;
                    }
...
msd::InputResolver::~InputResolver()
{
