...
{
    auto constexpr NUM_MODIFIERS = 11;
    int32_t constexpr modifiers[NUM_MODIFIERS] = {
        KEY_RIGHTSHIFT,
        KEY_LEFTSHIFT,
        KEY_RIGHTALT,
        KEY_LEFTALT,
    ...
    for (auto i = 0; i < NUM_MODIFIERS; i++)
    {
        if (modifiers[i] == scan_code)
            return true;
    }
...
{
    Synchronised<bool> should_disable_if_two_keys_are_pressed_together = true;
        ...
        input::EventBuilder* builder)
    {
        ...
        if (is_up_event)
        {
            for (auto const& pending : pending)
            {
                dispatcher(builder->key_event(
                    std::nullopt,
            ...
            case mir_keyboard_action_down:
            {
                ...
                if (!self->is_temporarily_disabled)
                {
                    self->is_temporarily_disabled =
                        *self->should_disable_if_two_keys_are_pressed_together.lock()
                        && self->held_count > 1;
                }
                break;
            }
...
{
    *self->should_disable_if_two_keys_are_pressed_together.lock() = on;
}
