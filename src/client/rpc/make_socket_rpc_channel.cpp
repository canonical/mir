                       ...
                       std::shared_ptr<mcl::TrustSessionControl> const& trust_session_control)
{
    if (fd_prefix.is_start_of(name))
    {
        ...
        return std::make_shared<MirSocketRpcChannel>(fd, map, disp_conf, rpc_report, lifecycle_control, trust_session_control);
    }

    return std::make_shared<MirSocketRpcChannel>(name, map, disp_conf, rpc_report, lifecycle_control, trust_session_control);
}
