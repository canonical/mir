...
public:
    GBMClientBuffer(std::shared_ptr<DRMFDHandler> const& drm_fd_handler,
                    std::shared_ptr<mir_client::MirBufferPackage>&& buffer_package,
                    geometry::Size size,
    ...
    std::shared_ptr<mir_client::MirBufferPackage> get_buffer_package() const;
...
private:
    ...
    const std::shared_ptr<mir_client::MirBufferPackage> creation_package;
