...
namespace gbm
{
class MesaNativeDisplayContainer
{
public:
    explicit MesaNativeDisplayContainer();
    virtual ~MesaNativeDisplayContainer() {}
    
    EGLNativeDisplayType create(MirConnection* connection);
    void release(EGLNativeDisplayType display);
    
    bool validate(EGLNativeDisplayType display);
};
}
}
} // namespace mir
