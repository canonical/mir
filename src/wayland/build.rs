...
fn main() {
            ...
            for interface in &interfaces {
                ...
                if let Some(info) = interface_info {
                    if !info.requests.is_empty() {
                        ...
                        dispatchers_rs_str.push_str(&format!("        use crate::protocols::{}::{}::Request as {}Request;\n", 
                            protocol_name.replace('-', "_"),
                            interface,
                            type_name
                        ));
    ...
    let wayland_type_to_rust = |arg_type: &str, allow_null: bool| -> String {
        match arg_type {
            "int" => "i32".to_string(),
            "uint" => "u32".to_string(),
            "fixed" => "i32".to_string(),  // Wayland fixed-point is represented as i32
