...
void mia::Lexicon::translate(const droidinput::InputEvent *android_event, MirEvent &mir_event)
{
    switch(android_event->getType())
    {
        case AINPUT_EVENT_TYPE_KEY:
        {
            ...
            mir_event.key.action = kev->getAction();
            mir_event.key.flags = kev->getFlags();
            mir_event.key.meta_state = kev->getMetaState();
        ...
        case AINPUT_EVENT_TYPE_MOTION:
        {
            ...
            mir_event.motion.flags = mev->getFlags();
            mir_event.motion.meta_state = mev->getMetaState();
            mir_event.motion.edge_flags = mev->getEdgeFlags();
            mir_event.motion.button_state = mev->getButtonState();
