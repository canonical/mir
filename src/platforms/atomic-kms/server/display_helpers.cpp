    ...
    mga::Quirks const& quirks)
{
    ...
    for(auto& device : devices)
    {
        ...
        else
        {
            ...
            case EINVAL:
                mir::log_warning(
                    "Failed to detect whether device {} supports KMS, but continuing anyway",
                    device.devnode());
                break;

            default:
                mir::log_warning("Unexpected error from drmCheckModesettingSupported(): {} ({}), but continuing anyway",
