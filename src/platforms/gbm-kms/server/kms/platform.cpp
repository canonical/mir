...
mgg::RenderingPlatform::RenderingPlatform(
    ...
    : RenderingPlatform(device.devnum(), gbm_device_for_udev_device(device, platforms))
{
}
...
mgg::RenderingPlatform::RenderingPlatform(
    ...
    : device{std::visit(gbm_device_from_hw{}, hw)},
      ...
      dmabuf_provider{maybe_make_dmabuf_provider(devnum, device, share_ctx->egl_display(), std::make_shared<mg::EGLExtensions>(), egl_delegate)}
{
}
