...
std::shared_ptr<mg::MesaAuthExtension> load_extension(mg::PlatformAuthentication& context)
{
    ...
    if (!ext.is_set())
        BOOST_THROW_EXCEPTION(std::runtime_error("No mesa_drm_auth extension. Perhaps the host is not using the gbm-kms platform"));
    return ext.value();
}
}

mgg::NestedAuthentication::NestedAuthentication(
...
void mgg::NestedAuthentication::auth_magic(drm_magic_t magic)
{
...
mir::Fd mgg::NestedAuthentication::authenticated_fd()
{
