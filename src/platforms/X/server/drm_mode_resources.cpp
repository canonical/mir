...
mgx::DRMModeResources::DRMModeResources(int drm_fd)
    : drm_fd{drm_fd},
...
void mgx::DRMModeResources::for_each_connector(std::function<void(DRMModeConnectorUPtr)> const& f) const
{
...
void mgx::DRMModeResources::for_each_encoder(std::function<void(DRMModeEncoderUPtr)> const& f) const
{
...
void mgx::DRMModeResources::for_each_crtc(std::function<void(DRMModeCrtcUPtr)> const& f) const
{
...
size_t mgx::DRMModeResources::num_connectors()
{
    return resources->count_connectors;
}

size_t mgx::DRMModeResources::num_encoders()
{
    return resources->count_encoders;
}

size_t mgx::DRMModeResources::num_crtcs()
{
    return resources->count_crtcs;
}

mgx::DRMModeConnectorUPtr mgx::DRMModeResources::connector(uint32_t id) const
{
    auto connector_raw = drmModeGetConnector(drm_fd, id);
    return DRMModeConnectorUPtr{connector_raw, ConnectorDeleter()};
}

mgx::DRMModeEncoderUPtr mgx::DRMModeResources::encoder(uint32_t id) const
{
    auto encoder_raw = drmModeGetEncoder(drm_fd, id);
    return DRMModeEncoderUPtr{encoder_raw, EncoderDeleter()};
}

mgx::DRMModeCrtcUPtr mgx::DRMModeResources::crtc(uint32_t id) const
{
