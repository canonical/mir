...
#ifndef MIR_GRAPHICS_X_GBM_BUFFER_H_
#define MIR_GRAPHICS_X_GBM_BUFFER_H_
...
public:
    GBMBuffer(std::shared_ptr<gbm_bo> const& handle,
              uint32_t bo_flags,
              std::unique_ptr<mesa::BufferTextureBinder> texture_binder);
...
private:
    ...
    uint32_t bo_flags;
    std::unique_ptr<mesa::BufferTextureBinder> const texture_binder;
    int prime_fd;
};
...
#endif // MIR_GRAPHICS_X_GBM_BUFFER_H_
