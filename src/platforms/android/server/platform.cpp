...
std::shared_ptr<mga::NativeWindowLogger> make_native_window_logger(mo::Option const& options)
{
    if (options.is_set(fb_native_window_report_opt) &&
        options.get<std::string>(fb_native_window_report_opt) == mo::log_opt_value)
        return std::make_shared<mga::ConsoleNativeWindowLogger>();
    else 
        return std::make_shared<mga::NullNativeWindowLogger>();
}
...
mga::Platform::Platform(
    ...
    std::shared_ptr<mga::NativeWindowLogger> const& native_window_logger,
        ...
        std::shared_ptr<mg::GLConfig> const& gl_config)
{
    ...
    return mir::make_module_ptr<mga::Display>(
            display_buffer_builder, program_factory, gl_config, display_report, native_window_logger, overlay_option);
}
    ...
    std::shared_ptr<mir::graphics::DisplayReport> const& display_report)
{
    ...
    return mir::make_module_ptr<mga::Platform>(
        ...
        make_native_window_logger(*options),
