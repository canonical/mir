...
char const* const gralloc_can_be_safely_closed_opt = "enable-gralloc-can-be-safely-closed-quirk";
...
unsigned int gralloc_can_be_safely_closed_for(std::string const& device_name, bool quirk_enabled)
{
...
mga::DeviceQuirks::DeviceQuirks(PropertiesWrapper const& properties)
    : device_name(determine_device_name(properties)),
      ...
      gralloc_can_be_safely_closed_(gralloc_can_be_safely_closed_for(device_name, true)),
...
mga::DeviceQuirks::DeviceQuirks(PropertiesWrapper const& properties, mo::Option const& options)
    : device_name(determine_device_name(properties)),
      ...
      gralloc_can_be_safely_closed_(gralloc_can_be_safely_closed_for(device_name, options.get(gralloc_can_be_safely_closed_opt, true))),
...
bool mga::DeviceQuirks::gralloc_can_be_safely_closed() const
{
    return gralloc_can_be_safely_closed_;
}
...
void mga::DeviceQuirks::add_options(boost::program_options::options_description& config)
{
    config.add_options()
        ...
        (gralloc_can_be_safely_closed_opt,
         boost::program_options::value<bool>()->default_value(true),
