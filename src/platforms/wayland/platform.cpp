...
mgw::Platform::Platform(struct wl_display* const wl_display, std::shared_ptr<mg::DisplayReport> const& report) :
    ...
    report{report},
    target_{std::make_shared<WlDisplayTarget>(make_initialised_egl_display(wl_display))}
{
}
...
{
    return mir::make_module_ptr<mgw::Display>(wl_display, target_, gl_config, report);
}

auto mgw::Platform::target() -> std::shared_ptr<DisplayTarget>
{
    return target_;
}
