            ...
            try
            {
                ...
                case mu::Monitor::ADDED:
                {
                    ...
                    if (device.devnode())
                    {
                        ...
                        if (std::strncmp(workaround_device->sysname(), "event", strlen_c("event")) != 0)
                        {
                            return;
                        }
                ...
                case mu::Monitor::REMOVED:
                {
                    ...
                    for (auto const& input_device : this->devices)
                    {
                        ...
                        if (device_udev)
                        {
                            if (std::strcmp(device.syspath(), udev_device_get_syspath(device_udev)) == 0)
                            {
                                libinput_path_remove_device(input_device->device());
                            }
                        }
...
void mie::Platform::device_added(libinput_device* dev)
{
    ...
    try
    {
        ...
        log_info("Opened device: {}", describe(dev).c_str());
    }
    catch(...)
    {
        log_error("Failure opening device {}", describe(dev).c_str());
    }
}
...
void mie::Platform::device_removed(libinput_device* dev)
{
    ...
    log_info("Removed device: {}", describe(dev).c_str());
}
