...
bool me::WindowManager::handle(MirKeyEvent const& kev)
{
    // TODO: Fix android configuration and remove static hack ~racarr
    static bool display_off = false;

    if (kev.action != mir_key_action_down)
        return false;

    auto modifiers = kev.modifiers;
    auto scan_code = kev.scan_code;
...
bool me::WindowManager::handle(MirMotionEvent const& mev)
{
    bool handled = false;
    geometry::Point cursor = average_pointer(mev);

    // FIXME: https://bugs.launchpad.net/mir/+bug/1311699
    MirMotionAction action = static_cast<MirMotionAction>(mev.action & ~0xff00);

    float new_zoom_mag = 0.0f;  // zero means unchanged

    if (mev.modifiers & mir_key_modifier_meta &&
        action == mir_motion_action_scroll)
    {
        zoom_exponent += mev.pointer_coordinates[0].vscroll;
    ...
    int fingers = static_cast<int>(mev.pointer_count);
    ...
    if (app)
    {
        ...
        if (surf &&
            (mev.modifiers & mir_key_modifier_alt ||
             fingers >= 3))
        {
            geometry::Displacement pinch_dir;
            auto pinch_diam =
                measure_pinch(mev, pinch_dir);
            ...
            else if (action == mir_motion_action_scroll)
            {
                ...
                alpha += 0.1f *
                    mev.pointer_coordinates[0].vscroll;
                if (alpha < 0.0f)
                    alpha = 0.0f;
                else if (alpha > 1.0f)
                    alpha = 1.0f;
...
bool me::WindowManager::handle(MirEvent const& event)
{
    ...
    {
        return handle(event.key);
    }
    else if (event.type == mir_event_type_motion &&
             focus_controller)
    {
        return handle(event.motion);
    }
    return false;
}
