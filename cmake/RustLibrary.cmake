...
function(add_rust_cxx_library target)
  ...
  set(multi_value_args DEPENDS LIBRARIES INCLUDES CXX_IMPL_SOURCES)
  ...
  # C++ side implementations called from the generated bridge code.
  # They are added to the cxxbridge library so it is self-contained when
  # consumed by targets other than the demo, and to avoid undefined-reference
  # errors on linkers (e.g. clang/lld on Fedora) that cannot resolve back-
  # references from a static library into an executable's own object files.
  if(arg_CXX_IMPL_SOURCES)
    # The impl sources include <rust/cxx.h> which lives inside
    # cxxbridge_include_dir and is generated by the cargo build step.
    # Mark each impl source with an order-only dependency on the generated
    # cxxbridge header so Ninja compiles them only after cargo has run.
    foreach(impl_src IN LISTS arg_CXX_IMPL_SOURCES)
      set_property(SOURCE ${impl_src} APPEND PROPERTY OBJECT_DEPENDS ${cxxbridge_header})
    endforeach()
    target_sources(${target}-cxxbridge PRIVATE ${arg_CXX_IMPL_SOURCES})
  endif()
