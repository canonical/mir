...
{
    MOCK_METHOD1(create_surface, std::weak_ptr<msh::BasicSurface>(msh::SurfaceCreationParameters const&));
    MOCK_METHOD1(destroy_surface, void(std::weak_ptr<msh::BasicSurface> const&));
    MOCK_METHOD1(raise, void(std::weak_ptr<msh::BasicSurface> const&));
};
}
...
TEST(SurfaceController, create_and_destroy_surface)
{
    ...
    std::weak_ptr<msh::BasicSurface> null_surface;
...
TEST(SurfaceController, raise_surface)
{
    ...
    controller.raise(std::weak_ptr<msh::BasicSurface>());
}
