...
TEST_P(OutputManagerTransformTest, output_transform_to_orientation_and_mirror_mode)
{
    ...
    EXPECT_THAT(result, Ne(std::nullopt));
...
TEST_F(OutputManagerTest, output_transform_returns_nullopt_when_invalid)
{
    auto const result = mf::OutputManager::from_output_transform(123);
