...
namespace
{
    ...
    {
        sleep_one_frame();
        auto buffer = bundle.compositor_acquire(0);
...
TEST_F(SwitchingBundleTest, is_really_synchronous)
{
    for (int nbuffers = 1; nbuffers <= 5; nbuffers++)
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            auto second_monitor = bundle.compositor_acquire(0);
...
TEST_F(SwitchingBundleTest, framedropping_clients_never_block)
{
    for (int nbuffers = 2; nbuffers <= 5; nbuffers++)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            {
                bundle.compositor_release(bundle.compositor_acquire(0));
                sleep_one_frame();
            }

            auto compositor = bundle.compositor_acquire(0);
...
TEST_F(SwitchingBundleTest, compositor_acquire_never_blocks)
{
    for (int nbuffers = 1; nbuffers <= 5; nbuffers++)
    {
        ...
        for (int i = 0; i < N; i++)
            buf[i] = bundle.compositor_acquire(0);
...
TEST_F(SwitchingBundleTest, compositor_release_verifies_parameter)
{
    for (int nbuffers = 2; nbuffers <= 5; nbuffers++)
    {
        ...
        auto compositor1 = bundle.compositor_acquire(0);
...
TEST_F(SwitchingBundleTest, overlapping_compositors_get_different_frames)
{
    ...
    for (int nbuffers = 2; nbuffers <= 5; nbuffers++)
    {
        ...
        compositor[0] = bundle.compositor_acquire(0);

        sleep_one_frame();
        bundle.client_release(bundle.client_acquire());
        compositor[1] = bundle.compositor_acquire(0);
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            bundle.client_release(bundle.client_acquire());
            sleep_one_frame();
            compositor[oldest] = bundle.compositor_acquire(0);
        }
...
namespace
{
                                    ...
                                    std::atomic<bool> &done)
    {
        while (!done)
        {
            auto buf = bundle.compositor_acquire(0);
            sleep_one_frame();
