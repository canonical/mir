...
class MockPlatform : public mg::Platform
{
 public:
    ...
    MOCK_CONST_METHOD2(arrange_buffer_ipc, void(mg::BufferIPCPacker*, mg::Buffer const*));
...
TEST_F(SessionMediatorTest, session_only_sends_mininum_information_for_buffers)
{
    using namespace testing;
...
TEST_F(SessionMediatorTest, session_with_multiple_surfaces_only_sends_needed_buffers)
{
...

    {
        ...
        EXPECT_CALL(*graphics_platform, arrange_buffer_ipc(_, stubbed_session->mock_buffer.get()))
            .Times(4);
...
TEST_F(SessionMediatorTest, fully_packs_buffer_for_create_screencast)
{
    ...
    EXPECT_CALL(*graphics_platform, arrange_buffer_ipc(_, &stub_buffer));
