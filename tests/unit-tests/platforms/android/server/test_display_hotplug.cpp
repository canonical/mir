...
struct DisplayHotplug : ::testing::Test
{
    struct StubHwcConfig : public mga::HwcConfiguration
    {
        ...
        mga::ConfigChangeSubscription subscribe_to_config_changes(
            std::function<void()> const& cb, std::function<void(mga::DisplayName, mg::Timestamp)> const&) override
        {
            hotplug_fn = cb;
            return {};
        }
    ...
    struct WrappingConfig : public mga::HwcConfiguration
    {
        ...
        mga::ConfigChangeSubscription subscribe_to_config_changes(
            std::function<void()> const& hotplug, std::function<void(mga::DisplayName, mg::Timestamp)> const& vsync) override
        {
