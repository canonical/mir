...
struct LibInputDevice : public ::testing::Test
{
    ...
    void setup_pointer_configuration(libinput_device* dev, double accel_speed, MirPointerHandedness handedness, MirPointerAccelerationProfile profile)
    {
        ...
        ON_CALL(mock_libinput, libinput_device_config_accel_get_profile(dev))
            .WillByDefault(Return((profile == mir_pointer_acceleration_profile_constant) ?
...
{
    setup_pointer_configuration(mouse.device(), 1, mir_pointer_handedness_right, mir_pointer_acceleration_profile_constant);
    ...
    EXPECT_THAT(ptr_settings.acceleration_profile, Eq(mir_pointer_acceleration_profile_constant));
