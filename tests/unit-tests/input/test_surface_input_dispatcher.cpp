...
TEST_F(SurfaceInputDispatcher, motion_synthesised_when_surface_under_cursor_resizes)
{
    FakePointer pointer;
    dispatcher.start();
...

    {
        ...
        EXPECT_CALL(*surface, consume(mt::PointerEnterEvent())).Times(1);
        // Resize triggers a synthesised motion so the client can refresh its cursor image
        // (no leave/enter pair — the pointer never left the surface)
        EXPECT_CALL(*surface, consume(mt::PointerMovementEvent())).Times(1);
    }
