...
TEST(TouchEventProperties, touch_count_taken_from_pointer_count)
{
    ...
    auto tev = mir_input_event_get_touch(mir_event_get_input_event(&old_ev));
    EXPECT_EQ(pointer_count, mir_touch_count(tev));
}
...
TEST(TouchEventProperties, touch_id_comes_from_pointer_coordinates)
{
    ...
    auto tev = mir_input_event_get_touch(mir_event_get_input_event(&old_ev));
    EXPECT_EQ(touch_id, mir_touch_id(tev, 0));
}
...
TEST(TouchEventProperties, down_and_up_actions_are_taken_from_old_event)
{
    ...
    auto tev = mir_input_event_get_touch(mir_event_get_input_event(&old_ev));
    EXPECT_EQ(mir_touch_action_down, mir_touch_action(tev, 0));
}
...
TEST(TouchEventProperties, touch_up_down_applies_only_to_masked_action)
{
    ...
    auto tev = mir_input_event_get_touch(mir_event_get_input_event(&old_ev));
    EXPECT_EQ(mir_touch_action_change, mir_touch_action(tev, 0));
    EXPECT_EQ(mir_touch_action_up, mir_touch_action(tev, 1));
    EXPECT_EQ(mir_touch_action_change, mir_touch_action(tev, 2));
}
...
TEST(TouchEventProperties, tool_type_copied_from_old_pc)
{
    ...
    auto tev = mir_input_event_get_touch(mir_event_get_input_event(&old_ev));
    EXPECT_EQ(mir_touch_tooltype_unknown, mir_touch_tooltype(tev, 0));
    EXPECT_EQ(mir_touch_tooltype_finger, mir_touch_tooltype(tev, 1));
    EXPECT_EQ(mir_touch_tooltype_stylus, mir_touch_tooltype(tev, 2));
}
...
TEST(TouchEventProperties, axis_values_used_by_qtmir_copied)
{
    ...
    auto tev = mir_input_event_get_touch(mir_event_get_input_event(&old_ev));
    EXPECT_EQ(x_value, mir_touch_axis(tev, 0, mir_touch_axis_x));
    EXPECT_EQ(y_value, mir_touch_axis(tev, 0, mir_touch_axis_y));
    EXPECT_EQ(touch_major, mir_touch_axis(tev, 0, mir_touch_axis_touch_major));
    EXPECT_EQ(touch_minor, mir_touch_axis(tev, 0, mir_touch_axis_touch_minor));
    EXPECT_EQ(pressure, mir_touch_axis(tev, 0, mir_touch_axis_pressure));
    EXPECT_EQ(size, mir_touch_axis(tev, 0, mir_touch_axis_size));
}
