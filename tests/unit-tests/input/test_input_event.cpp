...
MirMotionEvent a_motion_ev(int device_class = AINPUT_SOURCE_UNKNOWN)
{
    ...
    motion_ev.source_id = device_class;
...
TEST(KeyInputEventProperties, up_and_down_actions_copied_from_old_style_event)
{
    ...
    old_ev.action = mir_keyboard_action_down;
    ...
    old_ev.action = mir_keyboard_action_up;
...
TEST(KeyInputEventProperties, keycode_scancode_and_modifiers_taken_from_old_style_event)
{
    ...
    auto old_ev = a_key_ev();
    old_ev.key_code = key_code;
    old_ev.scan_code = scan_code;
    old_ev.modifiers = modifiers;
...
TEST(TouchEventProperties, touch_count_taken_from_pointer_count)
{
    ...
    old_ev.pointer_count = pointer_count;
...
TEST(TouchEventProperties, touch_id_comes_from_pointer_coordinates)
{
    ...
    old_ev.pointer_count = 1;
    old_ev.pointer_coordinates[0].id = touch_id;
...
TEST(TouchEventProperties, down_and_up_actions_are_taken_from_old_event)
{
    auto old_ev = a_motion_ev(AINPUT_SOURCE_TOUCHSCREEN);
    old_ev.pointer_count = 1;
    old_ev.pointer_coordinates[0].action = mir_touch_action_change;
...
TEST(TouchEventProperties, tool_type_copied_from_old_pc)
{
    ...
    old_ev.pointer_coordinates[0].tool_type = mir_touch_tooltype_unknown;
    old_ev.pointer_coordinates[1].tool_type = mir_touch_tooltype_finger;
    old_ev.pointer_coordinates[2].tool_type = mir_touch_tooltype_stylus;
...
TEST(PointerInputEventProperties, button_state_translated)
{
    ...
    old_ev.buttons = mir_pointer_button_primary;
    ...
    old_ev.buttons |=  mir_pointer_button_secondary;
...
TEST(PointerInputEventProperties, axis_values_copied)
{
    ...
    auto old_ev = a_motion_ev(AINPUT_SOURCE_MOUSE);
    old_ev.pointer_count = 0;
    old_ev.pointer_coordinates[0].x = x;
    old_ev.pointer_coordinates[0].y = y;
    old_ev.pointer_coordinates[0].vscroll = vscroll;
    old_ev.pointer_coordinates[0].hscroll = hscroll;
