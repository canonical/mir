...
TEST_F(OutputBuilder, registers_hooks_immediately_after_hwc_creation)
{
    using namespace testing;
    std::shared_ptr<mga::HWCCallbacks> callbacks;
<<<<<<< TREE
    EXPECT_CALL(*mock_resource_factory, create_hwc_native_device());
    EXPECT_CALL(*hw_access_mock.mock_hwc_device, register_hooks(_))
        .Times(1)
=======
    EXPECT_CALL(*mock_wrapper, register_hooks(_))
>>>>>>> MERGE-SOURCE
...
TEST_F(OutputBuilder, can_handle_notifications_before_any_subscriptions)
{
    using namespace testing;
    std::shared_ptr<mga::HWCCallbacks> callbacks;
    EXPECT_CALL(*mock_wrapper, register_hooks(_))
...
TEST_F(OutputBuilder, can_dish_out_notifications)
{
    using namespace testing;
    ...
    factory.subscribe_to_callbacks(
        [](mga::DisplayName, int64_t){vsync_call_count++;}
