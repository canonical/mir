...
#include "src/platform-implementations/android/real_hwc_wrapper.h"
#include "src/platform-implementations/android/hwc_report.h"
#include "src/platform-implementations/android/hwc_common_device.h"
...
TEST_F(HwcWrapper, submits_correct_prepare_parameters)
{
    ...
    Sequence seq;
    EXPECT_CALL(*mock_report, report_list_submitted_to_prepare(Ref(list)))
        .InSequence(seq);
    EXPECT_CALL(*mock_device, prepare_interface(mock_device.get(), 1, _))
        .InSequence(seq)
        .WillOnce(Invoke(this, &HwcWrapper::display_saving_fn));
    EXPECT_CALL(*mock_report, report_prepare_done(Ref(list)))
        .InSequence(seq);
...
TEST_F(HwcWrapper, submits_correct_set_parameters)
{
    using namespace testing;
    Sequence seq;
    EXPECT_CALL(*mock_report, report_set_list(Ref(list)))
        .InSequence(seq);
