...
TEST_F(AndroidGraphicBufferBasic, returns_native_buffer_times_two)
{
    ...
    EXPECT_CALL(*mock_native_buffer, update_usage(acquire_fake_fence_fd1))
        .Times(1);
    EXPECT_CALL(*mock_native_buffer, update_usage(acquire_fake_fence_fd2))
        .Times(1);
    ...
    mga::Buffer buffer(mock_native_buffer, extensions);
    {
        ...
        native_resource->update_usage(acquire_fake_fence_fd1);
    }
    {
        auto native_resource = buffer.native_buffer_handle();
        EXPECT_EQ(mock_native_buffer, native_resource);
        native_resource->update_usage(acquire_fake_fence_fd2);
    }
}
