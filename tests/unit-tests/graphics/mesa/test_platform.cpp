...
TEST_F(MesaGraphicsPlatform, adding_options_corrects_legacy_option)
{
    using namespace testing;
    ...
    mg::add_platform_options(desc);
    EXPECT_THAT(getenv(bypass_string.c_str()), Eq(nullptr)); //should still be unset

    setenv(legacy_string.c_str(), "1", 1);
    mg::add_platform_options(desc); //overwrites MIR_SERVER_BYPASS
    EXPECT_THAT(getenv(bypass_string.c_str()), StrEq("1"));

    setenv(legacy_string.c_str(), "1", 0);
    mg::add_platform_options(desc); //now MIR_SERVER_BYPASS=1, so dont touch
    EXPECT_THAT(getenv(bypass_string.c_str()), StrEq("1"));
