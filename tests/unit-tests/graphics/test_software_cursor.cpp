...
namespace
{
struct HardwareBufferCursorImage : mg::CursorImage
{
    explicit HardwareBufferCursorImage(std::shared_ptr<mg::Buffer> buf)
        : buf{std::move(buf)}
    {
    }
    ...
    geom::Size size() const override { return buf->size(); }

    geom::Displacement hotspot() const override { return {2, 3}; }

    auto buffer() const -> std::shared_ptr<mg::Buffer> override { return buf; }

private:
    std::shared_ptr<mg::Buffer> const buf;
};
}

TEST_F(SoftwareCursor, uses_hardware_buffer_directly_without_cpu_allocation)
{
    using namespace testing;

    auto const hw_buffer = std::make_shared<mtd::StubBuffer>(
        mg::BufferProperties{geom::Size{32, 32}, mir_pixel_format_argb_8888, mg::BufferUsage::hardware});

    auto const cursor_image = std::make_shared<HardwareBufferCursorImage>(hw_buffer);

    // alloc_software_buffer should NOT be called for hardware buffer cursor images
    EXPECT_CALL(mock_buffer_allocator, alloc_software_buffer(_, _)).Times(0);

    cursor.show(cursor_image);

    // The renderable should use the hardware buffer directly
    EXPECT_THAT(cursor.renderable()->buffer(), Eq(hw_buffer));
}

TEST_F(SoftwareCursor, hardware_buffer_cursor_has_correct_size)
{
    using namespace testing;

    geom::Size const hw_size{48, 48};
    auto const hw_buffer = std::make_shared<mtd::StubBuffer>(
        mg::BufferProperties{hw_size, mir_pixel_format_argb_8888, mg::BufferUsage::hardware});

    auto const cursor_image = std::make_shared<HardwareBufferCursorImage>(hw_buffer);

    cursor.show(cursor_image);

    EXPECT_THAT(cursor.renderable()->screen_position().size, Eq(hw_size));
}
