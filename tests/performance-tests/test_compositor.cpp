...
std::string mir_bin_dir()
{
    ...
    if (len < 0)
        len = 0;
    path[len] = '\0';
    // Unlike dirname(), we wish to return a path with trailing '/'
...
TEST(CompositorPerformance, stuff)
{
    ...
    auto const server_cmd =
        bin_dir+"mir_demo_server --compositor-report=log";

    if (auto server_output = popen_with_timeout(server_cmd.c_str(),
