...
void PrintTo(MirInputEvent const& event, std::ostream *os)
{
    ...
    case mir_input_event_type_pointer:
    {
        ...
        for (auto const a :
             { mir_pointer_button_primary, mir_pointer_button_secondary, mir_pointer_button_tertiary,
        ...
        *os << "pointer_event(when=" << event_time << ", from=" << device_id
            << ", " << pointer_actions[mir_pointer_event_action(pointer_event)]
...
void PrintTo(MirEvent const& event, std::ostream *os)
{
    ...
    {
        char const orientations[][9] = {"normal", "left", "inverted", "right"};
        *os << "prompt_session_state_change_event("
            << orientations[mir_orientation_event_get_direction(mir_event_get_orientation_event(&event))] << ")";
        break;
    }
