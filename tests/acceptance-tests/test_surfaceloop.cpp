...
TEST_F(DefaultDisplayServerTestFixture, surfaces_have_distinct_ids)
{
    struct Client : ClientConfigCommon
    {
        void exec()
        {
            ...
            EXPECT_NE(
                mir_debug_surface_get_id(ssync[0].surface),
                mir_debug_surface_get_id(ssync[1].surface));
        ...
        void exec()
        {
            ...
            for (int i = 0; i != max_surface_count; ++i)
            {
                for (int j = 0; j != max_surface_count; ++j)
                {
                    if (i == j)
                        EXPECT_EQ(
                            mir_debug_surface_get_id(ssync[i].surface),
                            mir_debug_surface_get_id(ssync[j].surface));
                    else
                        EXPECT_NE(
                            mir_debug_surface_get_id(ssync[i].surface),
                            mir_debug_surface_get_id(ssync[j].surface));
