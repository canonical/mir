...
struct ServerConfigurationWrapping : mir_test_framework::HeadlessTest
{
    void SetUp() override
    {
        ...
        cursor_observer = server.the_cursor_observer_multiplexer();
        surface_stack = server.the_surface_stack();
    }

    std::shared_ptr<msh::Shell> shell;
    std::shared_ptr<mi::CursorObserverMultiplexer> cursor_observer;
...
TEST_F(ServerConfigurationWrapping, cursor_observer_is_of_wrapper_type)
{
    auto const my_cursor_observer = std::dynamic_pointer_cast<MyCursorObserverMultiplexer>(cursor_observer);

    EXPECT_THAT(my_cursor_observer, Ne(nullptr));
}
...
TEST_F(ServerConfigurationWrapping, can_override_cursor_observer_methods)
{
    ...
    EXPECT_CALL(*my_cursor_observer, cursor_moved_to(abs_x, abs_y)).Times(1);
    cursor_observer->cursor_moved_to(abs_x, abs_y);
}

TEST_F(ServerConfigurationWrapping, returns_same_cursor_observer_from_cache)
{
    ASSERT_THAT(server.the_cursor_observer_multiplexer(), Eq(cursor_observer));
}
