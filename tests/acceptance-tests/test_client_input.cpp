...
TEST_F(TestClientInput, clients_receive_many_button_events_inside_window)
{
    ...
    auto expect_buttons = [&](MirPointerButtons b) {
        EXPECT_CALL(first_client, handle_input(mt::ButtonDownEvent(0, 0, b)));
    };
    ...
    auto emit_button = [&](int button) {
        fake_mouse->emit_event(mis::a_button_down_event().of_button(button).with_action(mis::EventAction::Down));
    };
    emit_button(BTN_LEFT);
    emit_button(BTN_RIGHT);
    emit_button(BTN_MIDDLE);
    emit_button(BTN_FORWARD);
    emit_button(BTN_BACK);
...
TEST_F(TestClientInput, send_mir_input_events_through_surface)
{
    ...
    auto key_event = mir::events::make_event(MirInputDeviceId{0}, 0, mir_keyboard_action_down, 0, KEY_M,
