...
struct TestBounceKeys : miral::TestServer
{
    TestBounceKeys()
    {
        ...
        bounce_keys.delay(test_bounce_keys_delay)
            .on_enabled([this] { bounce_keys_enabled.raise(); })
    ...
    void SetUp() override
    {
        ...
        bounce_keys_enabled.wait();
    }

    miral::BounceKeys bounce_keys{true};
    mir::test::Signal bounce_keys_enabled, press_rejected;
...
TEST_P(TestDifferentDelays, subsequent_keys_rejected_if_in_within_delay)
{
    ...
    auto virtual_keyboard =
        test::add_test_device(input_device_registry, input_device_hub, mir::input::DeviceCapability::keyboard);
