...
1. For ambiguous internal references (for example multiple matches for one label), disambiguate with Sphinx's standard-domain label role.
   - Preferred: `` {ref}`my-label`  ``.
   - Example: use `` {ref}`release-notes`  `` instead of doc/path-style links.
1. For missing anchors, replace with a currently working URL/anchor from upstream docs.
1. For stable, known 403 responses from external sites, add a focused regex to `doc/sphinx/conf.py` `linkcheck_ignore`.
1. Verify in sequence:
