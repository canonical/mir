...
impl ServerExtension for SessionLockListener {
    ...
    fn apply(self: Box<Self>, runner: Pin<&mut miral_sys::ffi::MiralRunner>) {
        miral_sys::set_on_session_lock_callback(self.on_lock);
        miral_sys::set_on_session_unlock_callback(self.on_unlock);
        miral_sys::ffi::miral_runner_add_session_lock_listener(runner);
    }
}
