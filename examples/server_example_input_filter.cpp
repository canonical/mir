...
{
    auto event_time = mir_input_event_get_event_time(ev);
    auto tev = mir_input_event_get_touch(ev);
    auto tc = mir_touch_count(tev);

    std::cout << "Handline touch event time=" << event_time
              << " touch_count=" << tc << std::endl;
    for (unsigned i = 0; i < tc; i++)
    {
        auto id = mir_touch_id(tev, i);
        auto px = mir_touch_axis(tev, i, mir_touch_axis_x);
        auto py = mir_touch_axis(tev, i, mir_touch_axis_y);
