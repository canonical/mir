...
static void on_event(MirSurface *surface, const MirEvent *event, void *context)
{
    ...
    if (event_type == mir_event_type_input)
    {
        ...
        case mir_input_event_type_touch:
            tev = mir_input_event_get_touch(input_event);
            touch_count = mir_touch_count(tev);
            ended = touch_count == 1 &&
                    (mir_touch_action(tev, 0) == mir_touch_action_up);
            break;
        ...
        else if (touch_count)
        {
            ...
            for (p = 0; p < touch_count; p++)
            {
                ...
                if (tev != NULL)
                {
                    x = mir_touch_axis(tev, p, mir_touch_axis_x);
                    y = mir_touch_axis(tev, p, mir_touch_axis_y);
                    float size = mir_touch_axis(tev, p, mir_touch_axis_size);
                    pressure = mir_touch_axis(tev, p, mir_touch_axis_pressure);
