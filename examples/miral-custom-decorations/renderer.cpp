...
std::mutex Renderer::Text::static_mutex;
std::weak_ptr<Renderer::Text> Renderer::Text::singleton;

auto Renderer::Text::instance() -> std::shared_ptr<Text>
{
...
void Renderer::Text::Impl::render(
    Pixel* buf,
    geom::Size buf_size,
    std::string const& text,
    geom::Point top_left,
...
void Renderer::Text::Impl::set_char_size(geom::Height height)
{
...
void Renderer::Text::Impl::rasterize_glyph(char32_t glyph)
{
...
void Renderer::Text::Impl::render_glyph(
    Pixel* buf,
    geom::Size buf_size,
    FT_Bitmap const* glyph,
    geom::Point top_left,
...
auto Renderer::Text::Impl::font_path() -> std::string
{
...
auto Renderer::Text::Impl::utf8_to_utf32(std::string const& text) -> std::u32string
{
...
namespace
{
auto init_button_drawing_functions(ButtonTheme button_icons)
{
    std::map<ButtonFunction, std::pair<Icon const, Icon::DrawingFunction const>> buttons;

    for(auto const& [button_function, icon]: button_icons)
    {
        // A little lookup table, more legible than a switch case?
        static auto const drawing_functions = std::map<ButtonFunction, Icon::DrawingFunction>{
...
void Renderer::update_state(WindowState const& window_state, InputState const& input_state)
{
...
auto Renderer::render_titlebar() -> std::optional<std::shared_ptr<mg::Buffer>>
{
...
auto Renderer::render_left_border() -> std::optional<std::shared_ptr<mg::Buffer>>
{
...
auto Renderer::render_right_border() -> std::optional<std::shared_ptr<mg::Buffer>>
{
...
auto Renderer::render_bottom_border() -> std::optional<std::shared_ptr<mg::Buffer>>
{
...
void Renderer::update_solid_color_pixels()
{
...
auto Renderer::make_buffer(
    uint32_t const* pixels,
    geom::Size size) -> std::optional<std::shared_ptr<mg::Buffer>>
{
    if (!area(size))
    {
...
auto Renderer::alloc_pixels(geom::Size size) -> std::unique_ptr<uint32_t[]>
{
