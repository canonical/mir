...
FloatingWindowManagerPolicy::FloatingWindowManagerPolicy(
    ...
    MinimalWindowManagerV2(tools),
    spinner{spinner},
    decoration_provider{std::make_unique<DecorationProvider>()}
{
    this->focus_stealing(focus_stealing);
...
{
    if (MinimalWindowManagerV2::handle_pointer_event(event))
        return true;
...
bool FloatingWindowManagerPolicy::handle_touch_event(MirTouchEvent const* event)
{
    ...
    if (MinimalWindowManagerV2::handle_touch_event(event) || count != 3)
    {
        pinching = false;
        return false;
    }
...
{
    MinimalWindowManagerV2::advise_new_window(window_info);
...
{
    MinimalWindowManagerV2::handle_window_ready(window_info);
    keep_spinner_on_top();
}

void FloatingWindowManagerPolicy::advise_focus_gained(WindowInfo const& info)
{
    MinimalWindowManagerV2::advise_focus_gained(info);
    keep_spinner_on_top();
}
...
{
    if (MinimalWindowManagerV2::handle_keyboard_event(event))
        return true;
...
{
    auto parameters = MinimalWindowManagerV2::place_new_window(app_info, request_parameters);
...
void FloatingWindowManagerPolicy::handle_modify_window(WindowInfo& window_info, WindowSpecification const& modifications)
{
    ...
    MinimalWindowManagerV2::handle_modify_window(window_info, mods);
}
